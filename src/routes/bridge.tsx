...
import { recordCompletedTx } from "@/hooks/useTxHistory";
  ...
  const handleBridge = useCallback(async () => {
    ...
    try {
      ...
      if (result.state === "success") {
        // Open the success modal and fire a toast confirmation. The
        // toast is short-lived, the modal is the persistent celebration.
        setShowSuccessModal(true);
        toast.success(`Bridged ${amount} USDC to ${CHAINS.find(c => c.id === to)?.name ?? to}`, {
          description: "Funds have arrived on the destination chain.",
          duration: 6000,
        });

        // Record the bridge in /history as a completed entry. We use the
        // burn step's tx hash on the source chain as the canonical
        // identifier — that's what the user pays gas for and what they'll
        // see on the source chain's explorer. Bridges go straight to
        // "completed" because by the time `result.state === "success"`
        // every CCTP step (approve, burn, attestation, mint) is already
        // done.
        const burnStep = result.steps.find((s) => s.name === "burn" && s.txHash);
        if (burnStep?.txHash && address) {
          recordCompletedTx({
            txHash: burnStep.txHash as `0x${string}`,
            kind: "bridge",
            title: "Bridge",
            subtitle: `${amount} USDC: ${CHAINS.find(c => c.id === from)?.name ?? from} → ${CHAINS.find(c => c.id === to)?.name ?? to}`,
            symA: "USDC",
            symB: "USDC",
            user: address,
            chainId: CHAIN_ID_MAP[from],
          });
        }
        ...
        const errMsg = String(errStep?.error ?? "Bridge ended in an error state.");
        setBridgeError(errMsg);
        toast.error("Bridge failed", { description: errMsg });
      }
      ...
      const msg = typeof raw === "string" ? raw : String(raw ?? "Bridge failed. Please try again.");
      setBridgeError(msg);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        <div className="bento-card p-6">
          ...
          {bridgeError && (
            <div className="mt-4 rounded-xl border border-destructive/30 bg-destructive/5 p-4">
              <div className="flex items-start gap-3">
                <svg className="size-5 shrink-0 text-destructive" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <circle cx="12" cy="12" r="10" />
                  <path d="M12 8v4M12 16h.01" />
                </svg>
                <div className="flex-1">
                  <p className="text-sm font-semibold text-destructive">Bridge failed</p>
                  ...
                  <button
                    ...
                    className="mt-3 rounded-full bg-destructive/10 px-3 py-1 text-xs font-semibold text-destructive transition hover:bg-destructive/20"
                  >
                    Try again
                  </button>
                </div>
          ...
          {/* Bridge Progress (only while in-flight) */}
          {isBridging && (
            <div className="mt-4 rounded-xl border border-primary/30 bg-primary/5 p-4">
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                Attestation can take ~10–15 minutes. You can leave this tab —
                we'll keep going in the background.
              </p>
            </div>
          )}

          {/* Successful bridge — open the celebration modal. The inline
              card is intentionally minimal because the modal is the focal
              point. */}
          {!isBridging && bridgeResult?.state === "success" && (
            <div className="mt-4 flex items-center justify-between rounded-xl border border-primary/30 bg-primary/5 px-4 py-3">
              <div className="flex items-center gap-2 text-sm font-semibold text-primary">
                ...
                Last bridge: {bridgeResult.amount} USDC delivered
              </div>
              <button
      ...
      <BridgeSuccessModal
        ...
        fromChain={{
          id: from,
          name: CHAINS.find((c) => c.id === from)?.name ?? from,
        }}
        toChain={{
          id: to,
          name: CHAINS.find((c) => c.id === to)?.name ?? to,
        }}
        address={address}
